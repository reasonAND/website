...
const Navbar = ({ toggleTheme, isDarkTheme }) => {
  ...
  const navTexts = ["LOREM", "IPSUM", "DOLOR", "MERLIN"];

  return (
    <div
      ref={navbarRef}
      className="fixed w-[100vw] flex justify-between items-center py-[10px] px-4 sm:px-8 sm:text-[16px] text-[#f7ced7] z-10"
      style={{ background: `rgba(30, 41, 59, ${o}` }}
    >
      ...
      {/* desktop */}
      <div className="hidden nav-middle desktop sm:flex sm:w-[60%] h-[40px] sm:items-center sm:justify-evenly">
        {navTexts.map((title, index) => (
          <a
            key={index}
            href="https://www.youtube.com/watch?v=yzfoogsTatE"
            className="border-pink-300 rounded hover:border hover:bg-black py-[0.85%] px-[0.75%]"
          >
            {title}
          </a>
        ))}
      ...
      <div className="flex gap-[6px]">
        ...
        {/* mobile  */}
        <button
          className="mobile hamburger sm:hidden flex justify-center items-center"
          onClick={toggleMenu}
        >
          {isMenuOpen ? (
            <svg
        ...
        <div
          className={`menu absolute top-full left-0 w-full h-[100vh] bg-slate-800 px-4 sm:hidden ${
            isMenuOpen ? "" : "hidden"
          }`}
        >
              ...
              {navTexts.map((title, index) =>
                index === 0 ? (
                  <a
                    key={index}
                    href="https://www.youtube.com/watch?v=yzfoogsTatE"
                    className="hamburger-list border-pink-300 hover:border text-[12.5vw] py-2 px-1 mt-1"
                  >
                    {title}
                  </a>
                ) : (
                  <a
                    key={index}
                    href="https://www.youtube.com/watch?v=yzfoogsTatE"
                    className="hamburger-list border-pink-300 hover:border text-[12.5vw] py-2 px-1"
                  >
                    {title}
                  </a>
                )
